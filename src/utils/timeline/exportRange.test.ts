...
import { exportContentRange, occupiedExportFrames, playbackContentRange, resolveExportTimeline } from '@/utils/timeline/exportRange'
...
describe('export content range', () => {
  ...
  it('ignores hidden tracks so blank frames are not exported', () => {
    ...
    expect(resolveExportTimeline(project, { startFrame: 0, endFrame: 900 })).toEqual(Array.from({ length: 20 }, (_, index) => 10 + index))
  })

  it('returns no frames when the timeline has no visible content', () => {
    const project = createEmptyProject()
    expect(exportContentRange(project)).toBeNull()
    expect(resolveExportTimeline(project, { startFrame: 0, endFrame: 900 })).toEqual([])
  })
