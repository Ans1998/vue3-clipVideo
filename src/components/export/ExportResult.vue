...
<template>
  <Modal v-if="exporter.result" title="导出完成" closable @close="exporter.close">
    ...
    <ul class="result-list">
      ...
      <li>格式：{{ exporter.result.exporterId === 'jianying' ? (exporter.result.savedToDirectory ? '剪映草稿目录' : 'ZIP 草稿') : exporter.result.format.toUpperCase() }}</li>
      <li v-if="!exporter.result.savedToDirectory">文件大小：{{ formatBytes(exporter.result.blob.size) }}</li>
    </ul>
    ...
    <template #footer>
      ...
      <button v-if="exporter.result.savedToDirectory" type="button" class="primary-button" @click="exporter.close">完成</button>
      <button v-else type="button" class="primary-button" @click="exporter.download">下载文件</button>
    </template>
  </Modal>
</template>
